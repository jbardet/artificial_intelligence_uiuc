...
class TabQPolicy(QPolicy):
    def __init__(self, env, buckets, actionsize, lr, gamma, model=None):
        ...
        #print(env, buckets, actionsize, lr, gamma)
        if model is None :
    ...
    def td_step(self, state, action, reward, next_state, done):
        ...
        current_q =self.model[current_state][action]
        next_q = max(self.model[self.discretize(next_state)])
        future_reward = reward + self.gamma*next_q
        if done and next_state[0]>=0.5:
            reward, future_reward = 1.0, 1.0
        self.model[current_state][action] = current_q + self.lr*(future_reward-current_q)
        return np.power((current_q-future_reward), 2)
...
if __name__ == '__main__':
    ...
    policy = TabQPolicy(env, buckets=(30, 30), actionsize=actionsize, lr=0.15, gamma=args.gamma)
