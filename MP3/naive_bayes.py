...
def naiveBayes(train_set, train_labels, dev_set, smoothing_parameter, pos_prior):
    ...
    result = []
    nb_posit = sum(posit.values())
    nb_negat = sum(negat.values())

    for movie in dev_set :
        proba_pos = 0
        proba_neg = 0
        for word in movie :
            proba_pos += math.log((posit[word]+k)/(nb_posit+k*len(posit)))
            proba_neg += math.log((negat[word]+k)/(nb_negat+k*len(negat)))
        proba_pos = math.log(nb_posit / (nb_posit+nb_negat)) + proba_pos
        proba_neg = math.log(nb_negat / (nb_posit+nb_negat)) + proba_neg
